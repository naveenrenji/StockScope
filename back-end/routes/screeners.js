...
router.route("/general-news").get(async (req, res) => {
  let newsData = [];
  let temp, temp1;
...
router.route("/news/:query").get(async (req, res) => {
  let newsData = [];
  let temp, temp1;
  let currentDate = new Date();
  const formattedDate = currentDate.toLocaleDateString();
  ...
  try {
    ...
    let { data } = await axios.get(
      `https://bing-news-search1.p.rapidapi.com/news/search?q=${query}&setLang=EN&freshness=Day&textFormat=Raw&safeSearch=Off&originalImg=true&count=50`,
      {
        headers: {
          "X-BingApis-SDK": "true",
          "X-RapidAPI-Key":
