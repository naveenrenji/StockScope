...
router.route("/:name").get(async (req, res) => {
  ...
  try {
    ...
    let finhubb_data = await axios.get(
      `https://finnhub.io/api/v1/quote?symbol=${stockName}&token=${"cgvbft1r01qqk0dog72gcgvbft1r01qqk0dog730"}`
    );
...
router.route("/news/:name").get(async (req, res) => {
  ...
  try {
    ...
    let { data } = await axios.get(
      `https://finnhub.io/api/v1/company-news?symbol=${stockName}&from=${lastWeekDate}&to=${currentDate}&token=${"cgvbft1r01qqk0dog72gcgvbft1r01qqk0dog730"}`
    );
